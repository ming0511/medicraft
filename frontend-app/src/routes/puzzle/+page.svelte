<script lang="ts">
	...
	function pickMode(): Mode {
		return selMode === 'random' ? MODES[Math.floor(Math.random() * MODES.length)] : selMode as Mode;
	}
	...
	function nextQ() {
		...
		if (qMode === 'normal') {
			...
			const ans = t.etymology[hiddenIdx].meaningKo;
			const src = terms.flatMap(x => x.etymology.map(e => e.meaningKo));
...
{#if phase === 'home'}
	...
	<div class="space-y-2 mb-6">
		{#each [
			...
			{ icon: '✅', text: `${GOAL}문제를 풀면 클리어! 하트가 먼저 소진되면 게임 오버입니다.` },
		...
		<div class="flex-1 text-sm">
			...
			{:else}
				...
				<p class="text-xs mt-0.5" style="color:var(--muted);">
					정답: {qMode==='reverse' ? term?.etymology[hiddenIdx].meaningKo : term?.etymology.map(e=>e.part).join(' + ')}
				</p>
			{/if}
		</div>
