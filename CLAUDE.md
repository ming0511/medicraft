...
- `/ranking` 랭킹 — 시상대 + 순위 리스트. **실동기화됨**: 로그인 시 Supabase `profiles`(XP·닉네임·학교·캐릭터)에서 전체 사용자 랭킹을 읽고, 내 로컬 XP를 진입 시 upsert. 게스트/Supabase 미설정이면 로컬 RIVALS 시뮬레이션으로 폴백. 정렬·집계(전체/학교별)는 XP 기준. 스토어=`src/lib/stores/leaderboard.svelte.ts`, 마이그레이션=`supabase/migrations/0004_leaderboard.sql`(profiles 점수 컬럼 + 전체 읽기 RLS). ⟶ 추후 어근 마스터 수 기준으로 전환 여지
